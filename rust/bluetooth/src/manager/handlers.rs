...
impl BluetoothManager {
    pub(super) fn handle_command(&mut self, cmd: BluetoothCommand) {
        match cmd {
            ...
            BluetoothCommand::GetChipCountForTesting { responder } => {
                let _ = responder.send(Ok(self.chips.len()));
            }
        }
    }

    fn create_chip(&mut self, params: CreateChipParams) -> Result<u32, ChipError> {
    ...
    fn patch_chip(&mut self, params: PatchChipParams) -> Result<ChipState, ChipError> {
        let chip_entry = &mut self
            .chips
            .get_mut(&params.chip_id)
            .ok_or(ChipError::ChipNotFound(params.chip_id))?;
        chip_entry.chip.patch_chip(params.patch)
    }

    fn get_chip(&self, params: GetChipParams) -> Result<ChipState, ChipError> {
        let chip_entry =
            &self.chips.get(&params.chip_id).ok_or(ChipError::ChipNotFound(params.chip_id))?;
        chip_entry.chip.get_chip()
    }

    fn delete_chip(&mut self, params: DeleteChipParams) -> Result<(), ChipError> {
        if let Some(chip_entry) = self.chips.remove(&params.chip_id) {
