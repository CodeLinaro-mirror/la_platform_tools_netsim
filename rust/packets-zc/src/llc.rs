...
impl LlcHeader {
    ...
    pub fn parse(bytes: &[u8]) -> Option<ParseResult<LlcHeader>> {
...
impl SnapHeader {
    ...
    pub fn parse(bytes: &[u8]) -> Option<ParseResult<SnapHeader>> {
        Ref::from_prefix(bytes).ok()
    }
}
...
impl LlcSnapHeader {
    ...
    pub fn parse(bytes: &[u8]) -> Option<ParseResult<LlcSnapHeader>> {
        Ref::from_prefix(bytes).ok()
    }
}
