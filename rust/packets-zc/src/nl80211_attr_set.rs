...
use crate::nl80211_util::{self, NetlinkError};
use std::fmt;
...
impl<'a> Nl80211AttrPacket<'a> {
    ...
    pub fn to_owned(&self) -> Result<Nl80211AttrSet, NetlinkError> {
        let mut set = Nl80211AttrSet::default();
        set.attributes = self.attributes.to_vec();
        let mut attrs_to_parse = self.attributes;
        if self.attributes.len()
            >= nl80211_util::nla_align(std::mem::size_of::<crate::nl80211::GenlMsgHdr>())
        {
            let (hdr, rest) =
                zerocopy::Ref::<&[u8], crate::nl80211::GenlMsgHdr>::from_prefix(self.attributes)
                    .unwrap();
            if hdr.cmd == 0 {
                attrs_to_parse = rest;
            }
        ...
        for (hdr, payload) in nl80211_util::iter_nl_attrs(attrs_to_parse) {
            ...
            match attr_id {
                ...
                attr_id::IFACE_NAME => {
                    set.iface_name = Some(nl80211_util::parse_string_from_payload(payload)?)
                }
                attr_id::IFACE_TYPE => {
                ...
                attr_id::REG_ALPHA2 => {
                    set.reg_alpha2 = Some(nl80211_util::parse_string_from_payload(payload)?)
                }
                attr_id::CHANNEL => {
...
mod tests {
    ...
    fn test_attr_set_parse() {
        let packet: Vec<u8> = include_bytes!("../tests/test_data/hwsim_cmd_frame.csv")
            .iter()
            .map(|&b| b as u8)
            .collect();
        let attrs = Nl80211AttrSet::parse(&packet).unwrap();
