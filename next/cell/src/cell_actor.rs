...
pub const CLIENT_EVENT_ID: ChipId = ChipId(u32::MAX);
...
pub struct CellActor {
    ...
    pub event_receiver: Option<UnboundedReceiver<bytes::Bytes>>,
}

impl CellActor {
    pub fn new(device_client: DeviceClient) -> Self {
        // Create channel for internal events loopback
        let (event_tx, event_rx) = tokio::sync::mpsc::unbounded_channel::<bytes::Bytes>();
