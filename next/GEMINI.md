...
`bazel build @netsim//next/<component_name>` Example:
`bazel build @netsim//next/wifi-actor`

# Testing Instructions

To run tests for individual components, use the following command:
`bazel test @netsim//next/<component_name>:integration-test` Example:
...
Example:
`bazel test @netsim//next/ap-actor:integration-test --test_env=RUST_LOG=debug --nocache_test_results --runs_per_test=100`
