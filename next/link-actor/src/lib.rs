...
// ## Structure
//
// - [`entity`] - [`ActorEntity`](actor_framework::ActorEntity) implementation for [`Link`]
...
// It maintains a collection of `Link` objects, each representing a connection
// between two chips.
//
// ## Key Components
//
...
// *   **Flow**:
//     1.  **Frontend**: Calls `LinkClient::create(LinkCreate { sender, receiver, rssi })`.
//     2.  **Actor**: `LinkActor::handle_create` is called.
//         *   Validates chips exist in `chip_kind_map`.
//         *   **Duplicate Check**: Checks `lookup`. If found, returns `AlreadyExists`.
