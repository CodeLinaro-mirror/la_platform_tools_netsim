...
mod tests {
    ...
    fn test_beacon_pcap_json() {
        // Validation with tshark output still requires file paths usually,
        // OR we need to adjust validate_pcap_json to accept bytes if we want to be pure.
        // But validate_pcap_json might be using tshark CLI which needs file paths.
        // So for this integration test, we might need to resolve path correctly.
        // Let's assume validate_pcap_json works because it's existing pattern?
        // Wait, did it fail? The error log showed failure in `test_beacon_details`.
        // `test_beacon_pcap_json` was NOT listed in failures, but maybe it didn't run or passed?
        // Ah, `test_beacon_details` failed. `test_beacon_pcap_json` might have passed if it was skipped or run?
        // Let's fix test_beacon_details primarily.
    ...
    fn test_beacon_details() {
        ...
        // Using PcapReader would be better but let's stick to simple offset for this unit test
        // OR use PcapReader since we already have it available in the crate (though maybe not exposed to this test easily without pub use).
        // Actually, let's just use the `validate_pcap_json` approach which uses tshark,
        // BUT here we want to test OUR Rust parsing.
