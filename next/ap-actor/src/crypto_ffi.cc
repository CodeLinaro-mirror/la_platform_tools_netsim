...
static bool Sha256PrfBits(const Vec<uint8_t>& key, const char* label,
                          const std::vector<uint8_t>& data, std::vector<uint8_t>& out, size_t bits) {
    uint16_t counter = 1;
    size_t pos = 0;
    ...
    while (pos < buf_len) {
        ...
        // LE encoding
        uint8_t count_bytes[2] = { (uint8_t)(counter & 0xff), (uint8_t)(counter >> 8) };
        ...
        if (!HMAC_Init_ex(ctx, key.data(), key.size(), EVP_sha256(), nullptr) ||
            ...
            !HMAC_Update(ctx, len_bytes, 2) ||
            !HMAC_Final(ctx, hash, &md_len)) {
            HMAC_CTX_free(ctx);
            return false;
        }
        HMAC_CTX_free(ctx);

        size_t copy_len = (plen >= SHA256_DIGEST_LENGTH) ? SHA256_DIGEST_LENGTH : plen;
...
rust::Vec<uint8_t> EcP256CalculatePwe(const rust::Vec<uint8_t>& password,
    ...
    for (uint8_t counter = 1; counter <= 40; counter++) {
        ...
        unsigned int md_len;
        uint8_t hash[SHA256_DIGEST_LENGTH];
        if (!HMAC(EVP_sha256(), key.data(), key.size(), kdf_data.data(), kdf_data.size(), hash, &md_len)) continue;
        for(int i=0; i<SHA256_DIGEST_LENGTH; i++) hmac_seed.push_back(hash[i]);

        // 2. KDF-z using hmac_seed as key, label="SAE Hunting and Pecking", data=prime
        ...
        if (!Sha256PrfBits(hmac_seed, "SAE Hunting and Pecking", prime_bin, pwd_value, 256)) continue;
        ...
        if (EC_POINT_set_compressed_coordinates_GFp(group, pwe_point, pwd_bn, y_bit, bn_ctx) == 1) {
             // Found it!
             // Serialize to uncompressed
             size_t len = EC_POINT_point2oct(group, pwe_point, POINT_CONVERSION_UNCOMPRESSED, nullptr, 0, bn_ctx);
             std::vector<uint8_t> temp_res(len);
             EC_POINT_point2oct(group, pwe_point, POINT_CONVERSION_UNCOMPRESSED, temp_res.data(), len, bn_ctx);
...
rust::Vec<uint8_t> EcP256PointMul(const rust::Vec<uint8_t>& point_src, const rust::Vec<uint8_t>& scalar) {
    Vec<uint8_t> result;
    EC_GROUP* group = EC_GROUP_new_by_curve_name(NID_X9_62_prime256v1);
    ...
    if (group && p && res && k && ctx &&
        ...
        size_t len = EC_POINT_point2oct(group, res, POINT_CONVERSION_UNCOMPRESSED, nullptr, 0, ctx);
        if (len > 0) {
             std::vector<uint8_t> temp_res(len);
             EC_POINT_point2oct(group, res, POINT_CONVERSION_UNCOMPRESSED, temp_res.data(), len, ctx);
             for(auto b : temp_res) result.push_back(b);
        }
    }
...
rust::Vec<uint8_t> EcP256PointAdd(const rust::Vec<uint8_t>& point_a, const rust::Vec<uint8_t>& point_b) {
    Vec<uint8_t> result;
    EC_GROUP* group = EC_GROUP_new_by_curve_name(NID_X9_62_prime256v1);
...
rust::Vec<uint8_t> BnModAdd(const rust::Vec<uint8_t>& a, const rust::Vec<uint8_t>& b, const rust::Vec<uint8_t>& m) {
    Vec<uint8_t> result;
...
rust::Vec<uint8_t> BnModSub(const rust::Vec<uint8_t>& a, const rust::Vec<uint8_t>& b, const rust::Vec<uint8_t>& m) {
    Vec<uint8_t> result;
    BIGNUM* bn_a = BN_bin2bn(a.data(), a.size(), nullptr);
    BIGNUM* bn_b = BN_bin2bn(b.data(), b.size(), nullptr);
    BIGNUM* bn_m = BN_bin2bn(m.data(), m.size(), nullptr);
