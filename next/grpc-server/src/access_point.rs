...
fn ap_config_to_proto(id: u32, config: ApConfig) -> AccessPoint {
...
fn proto_to_ap_config(proto: AccessPoint) -> ApConfig {
    ...
    if !proto.bssid.is_empty() {
        if let Ok(bssid) = proto.bssid.parse::<netsim_packets::ethernet::MacAddr>() {
            ...
            log::warn!("Invalid BSSID in Create request: {}", proto.bssid);
        }
    }
    if proto.channel != 0 {
    ...
    for mac_str in &proto.mac_acl_list {
        if let Ok(mac) = mac_str.parse::<netsim_packets::ethernet::MacAddr>() {
            ...
            log::warn!("Invalid MAC in ACL list: {}", mac_str);
        }
    }
...
impl AccessPointService for AccessPointServiceImpl {
    fn create(
        ...
        let f = async move {
            ...
            let config = proto_to_ap_config(req.access_point.unwrap_or_default());

            match client.create_ap(id, config.clone()).await {
                Ok(new_id) => {
                    let resp = ap_config_to_proto(new_id, config);
                    sink.success(resp);
                }
                Err(e) => {
    ...
    fn get(&mut self, ctx: RpcContext, req: GetAccessPointRequest, sink: UnarySink<AccessPoint>) {
        ...
        let f = async move {
            match client.get_ap(req.id).await {
                Ok(Some(state)) => {
                    sink.success(ap_config_to_proto(req.id, state.config));
                }
                Ok(None) => {
                    sink.fail(grpcio::RpcStatus::with_message(
    ...
    fn update(
        ...
        let f = async move {
            ...
            let force_disconnect = if !req.force_disconnect.is_empty() {
                Some(req.force_disconnect.into())
            } else {
                None
            };
        ...
        let f = async move {
            match client.list_aps().await {
                Ok(aps) => {
                    ...
                    for ap in aps {
                        resp.access_points.push(ap_config_to_proto(ap.id.into(), ap.config));
                    }
                    sink.success(resp);
                }
                Err(e) => {
