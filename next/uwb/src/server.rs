...
pub struct Server {
    ...
    uci_senders: HashMap<ChipId, mpsc::Sender<Bytes>>,
}
...
impl Server {
    ...
    pub async fn run(mut self) {
        ...
        log::info!("UwbServer is shutdown");
    }
    ...
    async fn handle_command(
        ...
        match msg {
            ...
            ChipRequest::Delete { id, respond_to } => {
                log::info!("UwbServer: DeleteChip received for chip_id: {}", id);
                self.cleanup_chip(id, "delete_request").await;
                let _ = respond_to.send(Ok(()));
            }
            ChipRequest::Read { id, respond_to } => {
                log::info!(
                    "UwbServer: GetChip received for chip_id: {}. Active chips: {:?}",
                    id,
                    self.active_chips
                );
    ...
    fn handle_create(&mut self, mut params: ChipCreate) -> Result<(), ChipError> {
        ...
        log::info!("UwbServer: CreateChip received for chip_id: {}", chip_id);
