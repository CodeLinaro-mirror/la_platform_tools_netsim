...
async fn test_bluetooth_hci_reset() {
    ...
    let netsimd_handle = tokio::spawn(async {
        netsimd::run().await;
    });
    ...
    let socket_path = netsim_next::platform::get_runtime_dir().join("netsim.sock");
    let uds_path = socket_path.to_str().expect("Invalid UDS path");

    let transport = TransportType::uds(uds_path);
