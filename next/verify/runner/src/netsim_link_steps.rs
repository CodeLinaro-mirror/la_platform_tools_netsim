...
/// STEP: When ^@netsim links @(\S+) to @(\S+) by (\S+) RSSI (-?\d+) as
/// "([^"]+)"$
async fn netsim_create_link(
    ...
    req.link = MessageField::some(link);
...
/// STEP: Then ^@netsim should list a link with RSSI (-?\d+)$
async fn netsim_verify_link_rssi(w: &mut TestContext, expected_rssi: i32) {
    ...
    let client = w.get_or_create_grpc_client().expect("got grpc client");
    let links_resp = client.list_link(&empty::Empty::new()).expect("listed links");
...
/// STEP: When ^@netsim patches link (\S+) with RSSI (-?\d+)$
async fn netsim_patch_link(w: &mut TestContext, link_var: String, rssi: i32) {
    ...
    w.log_step("@netsim", "->", &format!("Patches link {} with RSSI {}", resolved_link_id, rssi));
...
/// STEP: When ^@netsim deletes link (\S+)$
...
fn find_link_id(
    ...
    let links_resp = client.list_link(&empty::Empty::new()).ok()?;
    links_resp
        .links
        .iter()
        .find(|link| {
...
fn get_chip_ids(
    ...
    let resp =
        client.list_device(&empty::Empty::new()).map_err(|e| anyhow!("gRPC error: {:?}", e))?;
    let sender_id =
        find_chip_id(&resp, sender_device, kind).ok_or_else(|| anyhow!("Sender chip not found"))?;
    let receiver_id = find_chip_id(&resp, receiver_device, kind)
        .ok_or_else(|| anyhow!("Receiver chip not found"))?;
    Ok((sender_id, receiver_id))
}

fn find_chip_id(resp: &frontend::ListDeviceResponse, device_name: &str, kind: &str) -> Option<u32> {
    resp.devices
        .iter()
...
include!(env!("NETSIM_LINK_STEPS_GLUE"));
