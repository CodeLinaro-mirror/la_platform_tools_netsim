...
// A struct that divide a name into several parts that meets rust's guidelines.
...
impl<'a> NameSpliter<'a> {
    fn new(s: &str) -> NameSpliter<'_> {
        NameSpliter { name: s.as_bytes(), pos: 0 }
    }
}
...
pub fn proto_path_to_rust_mod(path: &str) -> String {
    let without_extension = path.trim_end_matches(".proto");
